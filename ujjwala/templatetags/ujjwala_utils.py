...
@register.filter()
def has_disbursement_review_abc_form_permission(user):
	return can_review_disbursement_form_abc_permission(user)
...
@register.filter()
def has_process_change_cylinder_type_request_permission(user):
	return can_process_change_cylinder_request(user)
