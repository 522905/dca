...
def make_api_request(url, payload, data=None):
...
class VicidialService:
	...
	def login(self, user, user_pass, phone_login, phone_pass):
	...
	def dispo(self, agent_user, dispo_code):
		dispo_payload = {
			"user": self.user,
			"pass": self.password,
			"agent_user": agent_user,
	...
	def is_logged_in(self, agent_user):
		...
		if result["status"] != "error":
			...
			if agent_user in result["message"]:
				return {"status": "success", "message": f"{agent_user} is logged in."}
	...
	def logout(self, agent_user):
		...
		return make_api_request(VICIDIAL_AGENT_API, logout_payload)
	...
	def Add_leadsToVicidial(self, data, list_id="5555"):
		...
		if result["status"] == "error":
			print("Error in creating lead", result)
			return result
	...
	def is_user_delivery_boy(self, data):
		...
		phone_payload = {
			...
			"extension": data['phone_login'],
			"phone_login": data['phone_login'],
			"voicemail_id": data['phone_login'],
			"dialplan_number": data['phone_number'],
			"outbound_cid": data['phone_number'],
			"phone_pass": data['phone_pass'],
			"registration_password": data['phone_pass'],
			"phone_full_name": f"ext{data['phone_login']}",
		...
		list_payload = {
			...
			"user": self.user,
			"pass": self.password,
			"list_id": data['phone_number'][5:],
			"list_name": "Test API list",  # 2-30 characters
			"campaign_id": data["agent_user"],
			"active": "Y",
			"list_description": "Delivery list for test_ash campaign",
		}
		result = make_api_request(VICIDIAL_NON_AGENT_API, list_payload)
		if result["status"] == "error":
