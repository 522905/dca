...
class CustomerProfileListView(ListView):
	...
	def get_queryset(self):
		...
		queryset = queryset.annotate(
			last_order_date=F('salesorder__order_date')
		).filter(
			salesorder__order_date__isnull=False,
			salesorder__order_status__in=[
				SalesOrderStatusEnum.COMPLETED, SalesOrderStatusEnum.INVOICED,
				SalesOrderStatusEnum.INVOICING_IN_PROGRESS
			]
		).annotate(
...
class CancelAutoGeneratedSalesOrderListView(FormView):
	...
	def form_valid(self, form):
		for so in self.qs():
			start_process_return_auto_sales_order(so.id, so.parent.distributor_code)
