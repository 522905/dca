...
from ujjwala.forms import KitchenPreInspectionForm, AudioOnSafetyForm, PreviewPreInspectionForm, ChangeAddressForm
...
def upload_images_TusServer(file_url):
	...
	try:
		...
		logs_dir = os.path.join(os.getcwd(), 'tmp')
...
def WhatsappPreInspection(Inspection_data, unique_id, intent):
	...
	if intent == "main-gate":
		...
		if Tus_data.get("status") != True:
			return "main gate photo link not found"
	...
	if intent == "address_details":
		...
		if dataSet["complete"]:
			return Follow_Set_Submission(conn, unique_id, pi_obj)

	return "Invalid intent provided."


def Follow_Set_Submission(conn, unique_id, pi_obj):
	try:
