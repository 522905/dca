...
import django_rq
from communication_log.connection_botviews import My_clinder_status, cylinder_subsidy_status
from .ujjwala_botviews import check_ujjwala_status, WhatsappPreInspection
...
def webhook(request):
    ...
    if request.method == 'POST':
        try:
            ...
            elif intent_name == 'booking-Cylinder-custom':
                response_texts = My_clinder_status(user_input, session_id)

            elif intent_name == 'Subsidy_status':
                response_texts = cylinder_subsidy_status(session_id, user_input)
